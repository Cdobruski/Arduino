...
- **IA generativa via Groq:** modelo `llama-3.3-70b-versatile`, 14.400 requests/dia gratuitos, sem cartão de crédito.
...
Arduino/
├── core/
│   ├── ai_client.py       ← integração com Groq (llama-3.3-70b)
...
├── config.json            ← porta serial, baud rate e chave da IA
├── requirements.txt
...
2. Obtenha uma chave gratuita em **[console.groq.com](https://console.groq.com)** → *API Keys* → *Create API Key*

3. Cole a chave em `config.json`:
   ```json
   { "groq_api_key": "gsk_..." }
   ```

4. Ajuste a porta serial se necessário (padrão: `COM3`).

5. Carregue o firmware `Placa/codigoplaca.c` no Arduino via Arduino IDE.
...
> Se o Arduino não estiver conectado, o programa inicia normalmente — a IA ainda funciona.
...
O script verifica dependências, compila e gera:

```
dist/
├── DilemasEticos.exe
└── config.json        ← edite aqui para trocar porta COM ou chave da IA
```
